import * as level from "level-hyper"
...
interface ICreateReadStreamChainedForm {
    on: {
        ...
        (type: "close", callback: () => void): void
        (type: "end", callback: () => void): void
    }
}
...
interface ILevelDB {
    ...
    on: {
        ...
        (eventType: "opening", callback: ()=> void): void
        (eventType: "open", callback: ()=> void): void
        (eventType: "ready", callback: ()=> void): void
        (eventType: "closing", callback: ()=> void): void
        (eventType: "closed", callback: ()=> void): void
    }
}

const Database: ILevelDB = level(`${__dirname}/../data`)
